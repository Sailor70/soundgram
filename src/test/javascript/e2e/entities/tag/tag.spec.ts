...
describe('Tag e2e test', () => {
  ...
  it('should load Tags', async () => {
    ...
    expect(await tagComponentsPage.getTitle()).to.eq('soundgramApp.tag.home.title');
  });

  it('should load create Tag page', async () => {
    await tagComponentsPage.clickOnCreateButton();
    tagUpdatePage = new TagUpdatePage();
    expect(await tagUpdatePage.getPageTitle()).to.eq('soundgramApp.tag.home.createOrEditLabel');
  ...
  it('should create and save Tags', async () => {
    ...
    await promise.all([
      ...
      // tagUpdatePage.userSelectLastOption(),
    ]);
  ...
  it('should delete last Tag', async () => {
    ...
    expect(await tagDeleteDialog.getDialogTitle()).to.eq('soundgramApp.tag.delete.question');
